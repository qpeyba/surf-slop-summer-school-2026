...
| [SCR-006_Auth](SCR-006_Auth.md) | Тап «Выйти» (logout) | — |
...
│                                     │
│  ─────────────────────────────────  │
│  Выйти                              │  → logout (LOGIC-001) → SCR-006
│                                     │
├─────────────────────────────────────┤
...
| Кнопка «Выйти» | Logout: сброс токена (LOGIC-001) → SCR-006 | Да |
...
| Кнопка «Выйти» | Outline-кнопка, текст «Выйти» | Сброс токена (LOGIC-001) → переход на SCR-006 |
...
| Выйти из аккаунта | Кнопка «Выйти» | Tap | Сброс токена (LOGIC-001) → переход на SCR-006 |
...
| AC-006 | **Дано** авторизованный пользователь, **Когда** тап «Выйти», **Тогда** токен сбрасывается (LOGIC-001), переход на SCR-006 | P0 |
