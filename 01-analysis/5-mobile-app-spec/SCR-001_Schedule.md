...
├─────────────────────────────────────┤
│ [←] Пн 12 · Вт 13 · Ср 14 · ... [→]│  ← Полоса дат (горизонтальный скролл, 7 дней)
│          ● сегодня                  │
├─────────────────────────────────────┤
│ ┌─────────────────────────────────┐ │
│ │ 12 июля, Вс • 14:00            │ │
...
| Полоса дат | Горизонтальный скроллируемый ряд из 7 дат. Выбранная дата — акцентный цвет. Сегодня — точка под датой. Стрелки «← →» для перемотки | Да |
...
### 2. Полоса дат (горизонтальный скролл)
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
| Чип даты | «Пн 12», «Вт 13» и т.д. — 7 дат от сегодня | Локально (текущая дата + `filterFrom`) | — | Тап → выбор даты, перезапрос `listSlots` с `from=выбранная дата` |
| Выбранная дата | Подсвечена акцентным цветом | `filterFrom` (состояние) | — | — |
| Сегодня | Точка под числом (если не выбрана) | Локально | — | — |
| Стрелки «← →» | Перемотка полосы на неделю назад/вперёд | — | — | Тап → сдвиг диапазона на 7 дней |

**Логика:**
- Фильтрация: [LOGIC-005](00_Логики/LOGIC-005_Фильтрация_по_дате.md) — при выборе даты в полосе `from = выбранная дата`, `to = from + 7 дней`, перезапрос `listSlots`.
- При выборе даты в BS-004 полоса скроллится к выбранной дате.

### 3. Карточка класса (каждый элемент списка)
| Элемент | Описание | Источник данных | Валидация | Действие |
|---------|----------|-----------------|-----------|----------|
...
| Выбрать дату | Чип даты в полосе | Tap | `filterFrom = выбранная дата` → перезапрос `listSlots` |
| Перемотать даты | Стрелка «←» / «→» | Tap | Сдвиг диапазона полосы на 7 дней |
...
| AC-006 | **Дано** загружен экран, **Когда** тап по чипу даты в полосе, **Тогда** выбранная дата подсвечивается, `listSlots` перезапрашивается с `from=выбранная дата` | P0 |
| AC-007 | **Дано** загружен экран, **Когда** тап по стрелке «→», **Тогда** полоса сдвигается на 7 дней вперёд | P1 |
